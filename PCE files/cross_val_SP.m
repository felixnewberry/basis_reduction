...
%N_k = 20; %Number of sparsities to attempt
%K = round(linspace(1,min(floor(N/2),size(Psi,2)),N_k)); %Sparsities to attempt
%K = union(K,K);
K = 1:min(floor(N/2),size(Psi,2));
N_k = length(K);
if nargin < 3 
    weight = false;
end
...
% Uncomment this to see the K optimization curve
% figure
% semilogy(K,SP_errs,'o-')
